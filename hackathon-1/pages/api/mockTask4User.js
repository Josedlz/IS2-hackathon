...
(async (req, res) => {
	...
	try {
		...
		if (userCredentials.is_admin === true) {
            ...
            const project = await dbService.createProject("Proyecto Mock", "el proyecto mock se encargara de facilitar el mocking en los espacios de trabajo a nivel multinacional.")
			const user2project = await dbService.addUserToProject(email,project[0].id)
            ...
            const epic = await dbService.createEpic("Mock Epic: Mockeado y Testeado", 
            ...
            // const getprojectepics = await dbService.getProjectEpics(project[0].id)
             ...
             const story = await dbService.createStory("Mock Story: Presentar el Proyecto Fiscalizador", 
                                                        "empieza a las 6pm", 
                                                        epic[0].id)
            ...
            const task = await dbService.createTask(email, 
            ...
            return res.status(200).json({ msg: "Mock creation completed", 
                                    getproject, getepic, getstory, gettask });
		} else{
